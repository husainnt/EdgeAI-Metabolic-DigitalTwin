...
NOTE: reusable functions/classes (build_calibrated_t2d_patient, warm_start,
simulate_open_loop, T2DPancreaticController) are kept at module level so
other scripts can `from run_selective_warmstart_simglucose_hybrid import (...)`.
The single-patient script execution (loading Patient 1031's CSV, training,
plotting) is moved under `if __name__ == "__main__":` so importing this
module for its functions does NOT re-run the whole thing -- this matters a
...
# --- 3. PyTorch Multimodal Residual Model (also kept importable) ---
...
def _run_single_patient_1031_script():
    """
    The original single-patient Patient 1031 script -- moved into a function
    under the __main__ guard below so importing this module doesn't trigger it.
    """
...
if __name__ == "__main__":
    _run_single_patient_1031_script()